...
public class _2751_Robot_Collisions {
    ...
    public List<Integer> survivedRobotsHealths(int[] positions, int[] healths, String directions) {
        ...
        // Sort index based on the position values
        Arrays.sort(actualIndex, (a, b) -> Integer.compare(positions[a], positions[b]));

        Deque<Integer> stack = new ArrayDeque<>();

        for (int index : actualIndex) {
            boolean alive = true;

            while (!stack.isEmpty() && directions.charAt(stack.peek()) == 'R' && directions.charAt(index) == 'L') {
