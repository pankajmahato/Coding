...
public class _731_My_Calendar_II {
    ...
    List<int[]> doubleBookingList = new ArrayList<>();
...
//    }
    ...
    public boolean book(int startTime, int endTime) {
        ...
        for (int[] pair : doubleBookingList) {
            if (hasOverlap(pair[0], pair[1], startTime, endTime)) {
                return false;
            }
        }

        // 2. Add all double bookings
        for (int[] pair : bookingList) {
            if (hasOverlap(pair[0], pair[1], startTime, endTime)) {
                doubleBookingList.add(getOverlap(pair[0], pair[1], startTime, endTime));
            }
        }

        bookingList.add(new int[]{startTime, endTime});
